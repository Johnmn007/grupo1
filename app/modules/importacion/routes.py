...
@login_required
def importar_cursos():
    """Importar cursos desde archivo Excel/CSV con creación automática de ciclo"""
    try:
        if 'archivo' not in request.files:
            flash('No se seleccionó ningún archivo', 'danger')
            return redirect(url_for('importacion.index'))
        ...
        from app.models import Ciclo  # importar aquí para evitar referencias circulares

        cursos_importados = 0
        cursos_actualizados = 0

        for _, fila in df.iterrows():
            # Buscar o crear el ciclo automáticamente según el semestre
            ciclo = Ciclo.query.filter_by(codigo_ciclo=fila['semestre']).first()
            if not ciclo:
                ciclo = Ciclo(
                    nombre=f"Ciclo {fila['semestre']}",
                    codigo_ciclo=fila['semestre'],
                    fecha_inicio=datetime.utcnow(),
                    fecha_fin=datetime.utcnow(),
                    activo=True
                )
                db.session.add(ciclo)
                db.session.flush()  # obtener ciclo.id sin hacer commit
